...
const authorization = "";//access code
var data_set = "F-C0032-001"
    const urlstring = `https://opendata.cwb.gov.tw/api/v1/rest/datastore/${data_set}?Authorization=${authorization}&format=JSON`;
const paragraph = "=================================================";
axios.get(urlstring)
.then(response => {
   ...
   var NewTaipeirecord = json_parse.records.location[1];
	...
	/*write file*/
  /*  
	fs.writeFile(`${fullfilename}.txt`, rainfall_probability(response.data), error => {
		if(error != null){
